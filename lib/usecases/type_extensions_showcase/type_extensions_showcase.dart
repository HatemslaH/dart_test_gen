...
class TypeExtensionsShowcase {
  ...
  Iterable<NameTag> pairNameTagsIterable(String x, String y) =>
      <NameTag>[NameTag(x), NameTag(y)];

  /// Возврат [Map] со значениями [NameTag]; ключи фиксированы для стабильного снимка.
  Map<String, NameTag> pairNameTagMap(String x, String y) => {
        'first': NameTag(x),
        'second': NameTag(y),
      };

  /// Возврат [List] с элементами [CounterLabel]; аргументы — граничные `int` из генератора.
  List<CounterLabel> pairCounterLabels(int a, int b) => [
        CounterLabel(a, ''),
        CounterLabel(b, 'test'),
      ];
