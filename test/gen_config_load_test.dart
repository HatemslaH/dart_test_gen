...
void main() {
  ...
  group('const IoConfigReader().loadConfig — missing file', () {
    test('returns defaults when no dart_test_gen.yaml exists', () {
      final config = const IoConfigReader().loadConfig(tempDir.path);
  ...
  group('const IoConfigReader().loadConfig — invalid YAML', () {
  ...
  group('const IoConfigReader().loadConfig — keep_runner', () {
  ...
  group('const IoConfigReader().loadConfig — explicit configPath', () {
    test('uses provided configPath instead of default', () {
      ...
      final config = const IoConfigReader().loadConfig(tempDir.path, configPath: customPath);
      expect(config.keepRunner, isTrue);
      expect(config.defaults.maxCases, 42);
    });

    test('returns defaults when explicit configPath does not exist', () {
      final config = const IoConfigReader().loadConfig(
        tempDir.path,
  ...
  group('const IoConfigReader().loadConfig — per-method override inheritance', () {
